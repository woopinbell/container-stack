#!/usr/bin/env python3
# [INTV:ARCH] 이 파일은 컨테이너를 실제로 띄우지 않는 "정적" 검증이다(Makefile의 check-functional
# 타겟이 CI의 첫 스텝으로 이 파일을 --functional 인자와 함께 호출한다) — docker-compose.yml,
# Dockerfile, 셸 스크립트, 다른 파이썬 도구들의 소스 코드를 텍스트/정규식으로 훑어서 "이전에 내린
# 설계 결정이 그대로 남아있는지"를 확인한다. 예를 들어 특정 베이스 이미지 다이제스트, 체크섬 값,
# 심지어 다른 파일의 함수 이름까지 문자열로 못박아두고 검사하는데, 이는 리팩터링 도중 의도적으로
# 넣어둔 방어 로직이나 고정값이 조용히 사라지는 것을 잡아내기 위한 회귀 테스트 기법이다(실제 동작
# 검증은 tests/runtime_stack.py가 담당).
# [INTV:TRAP] 텍스트/정규식으로 "특정 문자열이 존재하는가"만 확인하는 방식은, 검사 대상 코드에
# 주석 한 줄만 끼어들어도 깨질 수 있다는 약점이 있다(위 validate_tools()의 Makefile 검사가 실제로
# 그렇게 깨졌던 사례가 bug-fix-report.md에 있다) — 정규식을 짤 때 "두 줄이 바로 인접해야 한다"는
# 가정(`\s+`로만 연결)은 최소한으로 쓰고, 가능하면 각 조건을 독립적인 검사로 나누는 편이 이런
# 종류의 회귀에 더 강하다.
# [INTV:TRAP] main()은 argparse 없이 그냥 고정된 순서로 모든 validate_*()를 실행한다 — 즉 Makefile의
# check-functional이 넘기는 `--functional` 인자는 이 스크립트 어디에서도 읽히지 않는, 사실상
# 죽은(no-op) 플래그다. 이 코드를 재구현/확장한다면 "--functional이 검사 범위를 좁혀줄 것"이라고
# 착각하기 쉬운데, 실제로는 인자 유무와 무관하게 항상 같은 8개 검증을 전부 돈다.
